Hooks.on("init", async () => {
	...
	game.settings.register("multi-roll", "hideOGButtons", {
		scope: "world",
		config: true,
	...
	game.settings.register("multi-roll", "persistentDamageInt", {
		scope: "world",
		config: true,
...
Hooks.on("preCreateChatMessage", (message) => {
	...
	message.updateSource({
		"flags.multi-roll.targets": Array.from(game.user.targets).map((target) => {
			return {
				id: target.id,
				name: target.name,
...
Hooks.on("renderChatMessage",
	async (message, html, data) => {
		...
		if (message.flags.persistent) {
			...
			targets = message.getFlag('multi-roll', 'targets')
		}
		...
		for (const [index, target] of targets.sort(function (x, y) {
			...
			const nameHTML = $(`<div data-visibility='${game.settings.get('multi-roll', 'hideNPCs') ? !target.hasPlayerOwner ? "gm" : "all" : "all"}' id='target-damage-target-name'>Target: ${target.name}</div>`)
		...
		if (game.settings.get('multi-roll', 'hideOGButtons') || (message.flags.persistent && game.settings.get('multi-roll', 'persistentDamageInt'))) html.find("div.chat-damage-buttons").first().remove();
	});
